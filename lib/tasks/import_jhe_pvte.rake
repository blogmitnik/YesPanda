...
task :import_jhe_pvte, [:filename] => :environment do
	prefer_stages = ["JHE-PVTe"]
	prefer_stages.each do |prefer_stage|
		puts "Start auto importing process for " + prefer_stage
		start_time = Time.now
		imported_files = []
		imported_main_files = []
		imported_mini_files = []
		...
		filtered_csvs.select do |file|
			filename = File.basename(file).gsub('_', '-')
			if (filename.include? latest_datetime) || (filename.include? date_today + "06")
		...
		filtered_csvs_from_base_and_latest_hour.select do |file|
	        ...
	        # Do nothing when CSV files contain no row data
	        	...
	        	if post = Post.find_by_title(model_stage)
	        		if (filename.include? "Sum-Main") && ((filename.include? latest_datetime) || (filename.include? date_today + "06"))
			    	...
			    	elsif (filename.include? "Sum-Min") && ((filename.include? latest_datetime) || (filename.include? date_today + "06"))
			        ...
			        elsif (filename.include? latest_datetime) || (filename.include? date_today + "06") # For handling 'Sum-All' and 'Sum-Config' csv files
			            Report.import_data(file, filename, post)
			            # Check total imported row counts
			            if file = YieldFile.find_by_file_name(filename)
			            	count = Report.where(post_id: post, yield_file_id: file).count
