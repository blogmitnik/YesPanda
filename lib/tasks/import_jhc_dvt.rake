...
task :import_jhc_dvt, [:filename] => :environment do
	...
	d = Dir.new('/Users/david/desktop/CSV_to_import/JHC')
	#Dir.chdir(d)
	...
	csvs.select do |file|
		...
		if filename.include? "JHC-DVT.csv"
    ...
    if imported_files.count > 0
    	Report.export_to_xml("JHC-DVT")
	end
	if imported_main_files.count > 0
    	ReportMain.export_to_xml("JHC-DVT")
    end
    if imported_mini_files.count > 0
		ReportMini.export_to_xml("JHC-DVT")
	end

	# Export blank XML files for no-row csv files
	if generate_dummy_xml
		Report.export_dummy_xml("JHC-DVT", latest_datetime)
	end
	if generate_dummy_main_xml
		ReportMain.export_dummy_xml("JHC-DVT", latest_datetime)
	end
	if generate_dummy_min_xml
		ReportMini.export_dummy_xml("JHC-DVT", latest_datetime)
	end
