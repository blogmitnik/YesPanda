...
task :import_jhh_ab, [:filename] => :environment do
	prefer_stages = ["JHH-AB", "JHH-IM-AB"]
	prefer_stages.each do |prefer_stage|
	    ...
	    mounted_folder = '/Users/david/desktop/CSV_to_import/JHH'
		...
		cur_date_hour = DateTime.parse(Time.now.to_s).strftime('%Y%m%d%H')
		csvs = Dir[ File.join(d, '**', '*.csv') ].reject { |p| !p.include?(cur_date_hour) }
