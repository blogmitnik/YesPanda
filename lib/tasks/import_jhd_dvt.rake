...
task :import_jhd_dvt, [:filename] => :environment do
	...
	d = Dir.new('/Users/david/desktop/CSV_to_import/JHD')
	#Dir.chdir(d)
	...
	csvs.select do |file|
		...
		if filename.include? "JHD-DVT.csv"
    ...
    if imported_files.count > 0
		Report.export_to_xml("JHD-DVT")
	end
	if imported_main_files.count > 0
    	ReportMain.export_to_xml("JHD-DVT")
    end
    if imported_mini_files.count > 0
		ReportMini.export_to_xml("JHD-DVT")
	end

	# Export blank XML files for no-row csv files
	if generate_dummy_xml
		Report.export_dummy_xml("JHD-DVT", latest_datetime)
	end
	if generate_dummy_main_xml
		ReportMain.export_dummy_xml("JHD-DVT", latest_datetime)
	end
	if generate_dummy_min_xml
		ReportMini.export_dummy_xml("JHD-DVT", latest_datetime)
	end
