...
task :clean_overdue_records_gl_dvt, [:filename] => :environment do
	puts 'Start the process deleting overdue records at ' + Time.now.to_s
	start_time = Time.now

	file_model_stage_name = "JHC-GL-DVT"
	...
	file_model_stage_name = "JHD-GL-DVT"
	post = Post.find_by_title(file_model_stage_name)
	if !post.nil?
		puts "Deleting " + file_model_stage_name + " overdue records..."
		Report.delete_overdue_records_beta(file_model_stage_name)
	...
	file_model_stage_name = "JHE-GL-DVT"
	post = Post.find_by_title(file_model_stage_name)
	if !post.nil?
		puts "Deleting " + file_model_stage_name + " overdue records..."
		Report.delete_overdue_records_beta(file_model_stage_name)
	...
	file_model_stage_name = "JHF-GL-DVT"
	post = Post.find_by_title(file_model_stage_name)
	if !post.nil?
		puts "Deleting " + file_model_stage_name + " overdue records..."
		Report.delete_overdue_records_beta(file_model_stage_name)
